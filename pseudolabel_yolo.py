...
class YoloPredictor:
    ...
    def __init__(self, cfg, checkpoint='bbox-yolo/yolov8n.pt', device='cuda'):
        self.cfg = cfg
        from ultralytics import YOLO

        # Load a model
        self.model = YOLO('bbox-yolo/yolov8n.pt')  # pretrained YOLOv8n model
    ...
    def __call__(self, original_image):
        ...
        with torch.no_grad():  # https://github.com/sphinx-doc/sphinx/issues/4258
            ...
            predictions = self.model([inputs],project='/bbox-yolo/save_dir', 
            name='testing_prediction')[0]
            return predictions

            # Run batched infer nce on a list of images
            #results = model(['datasets/my_examples/img1.png', 'datasets/my_examples/img2.png', 'datasets/my_examples/img3.png'])  # return a list of Results objects
            # results = model(['/cluster/work/cotterell/yemara/ml-4m/bbox-yolo/datasets/my_examples/img1.png', '/cluster/work/cotterell/yemara/ml-4m/bbox-yolo/datasets/my_examples/img2.png', '/cluster/work/cotterell/yemara/ml-4m/bbox-yolo/datasets/my_examples/img3.png'],
            # project='/cluster/work/cotterell/yemara/ml-4m/bbox-yolo/save_dir', 
...
def get_predictor(device="cuda:0"):
...
def save_pseudo_detection(classes_list, rgb_dir, save_dir, thresh=0.2, device="cuda:0", save_segmentation=True):

    predictor = get_predictor(device='cuda:0')
    ...
    for class_idx, class_id in enumerate(classes_list):
        ...
        for i, img_path in enumerate(tqdm(class_img_paths)):
            img_name = img_path.split('/')[-1].replace('.JPEG', '.json').replace('.jpg', '.json').replace('.png', '.json')
            save_path = os.path.join(save_dir, class_id, img_name)

            im = cv2.imread(img_path)
            outputs = predictor(im)
            annotations = annotate_image(outputs['instances'], predictor.metadata.thing_classes, thresh=thresh, save_segmentation=save_segmentation)
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--model_weights', type=str, 'bbox-yolo/yolov8n.pt'
    #                     default="/scratch/david/detectron2/checkpoints/coco_cascade_mask_rcnn_vitdet_h.pkl")
    #parser.add_argument('--model_weights', type=str, default="'bbox-yolo/yolov8n.pt'")
    #parser.add_argument('--rgb_dir', type=str, default='/datasets/imagenet_multitask/train/rgb')
    parser.add_argument('--rgb_dir', type=str, default='/store/swissai/a08/yemara/ml-4m/bbox-yolo/datasets/my_examples')
    parser.add_argument('--save_dir', type=str, default='bbox-yolo/save_dir')
    ...
    if folder_list is None:
        save_pseudo_detection(model_weights=args.model_weights, classes_list=None,
                              rgb_dir=args.rgb_dir, save_dir=args.save_dir, device=f'cuda:{args.device_id}',
                              thresh=args.detection_threshold, save_segmentation=args.save_segmentation)
    ...
    for gpu_idx in range(args.n_gpus):
        ...
        for proc_idx in range(args.n_proc_per_gpu):
            ...
            func = Process(target=partial(
                save_pseudo_detection,
                classes_list=classes_list, rgb_dir=args.rgb_dir, save_dir=args.save_dir,
