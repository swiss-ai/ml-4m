...
def parse_args():
    ...
    parser.add_argument("--test_ratio", type=float, required=False, default=1)
    parser.add_argument(
        "--conv_mode",
        type=str,
        required=False,
...
def load_model_and_dataset(
    ...
    dataset = VideoQABenchDataset(
        test_ratio=test_ratio, test_datasets=test_datasets, num_segments=num_frames
    )
...
def infer_videoqabench(
    ...
    conv.user_query(
        data_sample["question"], pre_query_prompt, post_query_prompt, is_mm=True
    )
...
def run(rank, args, world_size):
    ...
    for example in dataset:
        ...
        if task_type in dataset.data_list_info:
            ...
            res = {"pred": pred, "gt": gt, "task_type": task_type, **infos}
        else:
