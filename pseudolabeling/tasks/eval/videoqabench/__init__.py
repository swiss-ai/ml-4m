...
def chatgpt_eval(res, model="gpt-3.5-turbo-0125"):
    ...
    correct, score = check_ans_qa(
        question=question, pred=pred, gt=gt, task_type=task_type, model=model
...
class VideoQABenchDataset(EvalDataset):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if test_datasets is not None:
            data_list_info = {
                k: v for k, v in self.data_list_info.items() if k in test_datasets
            }
        else:
        ...
        for k, v in data_list_info.items():
            ...
            num_samples = (
                int(len(indexs) * self.test_ratio)
                if 0 < self.test_ratio <= 1
                else int(self.test_ratio)
            )
    ...
    def __getitem__(self, idx):
        ...
        for p in video_postfixs:
            video_path = os.path.join(
                self.data_list[idx]["prefix"], video_name + "." + p
            )
