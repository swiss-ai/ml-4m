...
class Conversation(EasyDict):
    ...
    def get_prompt(self):
        sep = (
            [self.sep for _ in self.roles] if isinstance(self.sep, str) else self.sep
    ...
    def assistant_response(
        self, response, pre_query_prompt=None, post_query_prompt=None
    ):
        if post_query_prompt is not None:
...
class EvalDataset(Dataset):
    ...
    def read_gif(self, video_path, bound=None, fps=25):
        ...
        if len(images_group) < self.num_segments:
            ...
            images_group = [image for _ in range(multiplier) for image in images_group][
                : self.num_segments
            ]
...
class ChatPllava:
    ...
    def upload_video(
        self, image, conv: Conversation, img_list: list[list], num_segments=None
    ):
        num_segments = (
            self.model.config.num_frames if num_segments is None else num_segments
        )
    ...
    def upload_img(self, image, conv, img_list):
        ...
        image_emb, _ = self.model.encode_img(
            img, "Observe the image and answer the question."
        )
