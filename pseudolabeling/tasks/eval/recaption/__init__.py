...
def chatgpt_eval(res, model="gpt-3.5-turbo-0125"):
    ...
    correct, score, num_tokens_openai = check_ans_recaption(
        pred=pred, gt=gt, task_type=task_type, model=model
    )  # acc is bool, score is given by chatgpt
    ...
    with Pool(7) as pool:
        ...
        result_list = [
            ...
            for res in tqdm.tqdm(
                pool.imap_unordered(func, result_list),
                total=len(result_list),
                desc="Language Chat Model Automated Evaluation...",
            )
        ]
...
class RecaptionSample(EasyDict):
    ...
    def get_info(self):
        ...
        return {"noisy_caption": self.caption}
...
class RecaptionDataset(EvalDataset):
    ...
    def __init__(self, *args, **kwargs):
        ...
        test_ratio = 1.0 if test_ratio is None else test_ratio
    ...
    def __getitem__(self, idx):
        ...
        for p in video_postfixs:
            video_path = os.path.join(
                self.data_list_info[task_type]["prefix"], video_name + "." + p
            )
        ...
        sample = self.data_list_info[task_type]["recaption_type"](
