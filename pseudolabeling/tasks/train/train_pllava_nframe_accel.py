...
def get_state_maybe_zero_3(
    named_params, keys_to_match=["lora_", "multi_modal_projector"]
):
    to_return = {
        k: t
...
def setup_model(config, find_unused_parameters=False):
    ...
    processor = PllavaProcessor.from_pretrained(
    ...
    model = PllavaForConditionalGeneration.from_pretrained(
        config.model.repo_id, config=model_config, torch_dtype=torch_dtype
    )

    if config.model.load_from_origin:
        with torch.no_grad():
            lm_model = AutoModelForCausalLM.from_pretrained(
                config.model.origin_llm,
                torch_dtype=torch_dtype,
                device_map="cpu",
            )
        with torch.no_grad():
            clip = AutoModel.from_pretrained(
                config.model.origin_vision,
                torch_dtype=torch_dtype,
                device_map="cpu",
            )
    ...
    if config.model.use_lora:
        ...
        kwargs = {}
        if (
            config.model.lora_target_modules is not None
            and len(config.model.lora_target_modules) > 0
        ):
    ...
    if config.model.pretrained_path is not None and not config.deepspeed:
        logger.info(
            ...
            + str(config.model.pretrained_path)
        )
        state_dict = {}
        save_fnames = os.listdir(config.model.pretrained_path)
        if "model.safetensors" in save_fnames:
            print(
                "Loading weight from", config.model.pretrained_path, "model.safetensors"
            )
            with safe_open(
                f"{config.model.pretrained_path}/model.safetensors",
                framework="pt",
                device="cpu",
            ) as f:
                for k in f.keys():
            ...
            for fn in save_fnames:
                if fn.startswith("model-0000"):
                    with safe_open(
                        f"{config.model.pretrained_path}/{fn}",
                        framework="pt",
                        device="cpu",
                    ) as f:
                        for k in f.keys():
...
def setup_optimizer_and_scheduler(config, model):
    ...
    )  # do you want to filter bias and bn?
...
class RandomMappingIterator:
    ...
    def __init__(self, train_loaders, media_types, resume_step=0):
        ...
        self.total_num_samples = sum(
            len(train_loader) for train_loader in self.train_loaders
        )
        self.weights = [
            len(loader) / self.total_num_samples for loader in train_loaders
        ]
    ...
    for sep in separators:
        ...
        for text in texts:
            if sep not in text:
                ...
                joint_strings = [
                    t
                    for pair in zip(split_strings[:-1], itertools.repeat(sep))
                    for t in pair
                ] + split_strings[-1:]
...
def preprocess(
    ...
    for sample in batch:
        ...
        tex = tex.replace(args.dataset_video_placeholder, IMAGE_TOKEN).replace(
            args.dataset_image_placeholder, IMAGE_TOKEN
        )
        ...
        for i, seg in enumerate(segs):
            seg_ignore = (
                False
                if seg == seps[1]
                else (True if i == 0 or seg in seps else seg_ignore)
            )  # not ignoring assistant, changing in sepecific situations
            current_ignore = (
                True if seg in seps else seg_ignore
            )  # serve for only this one iteration
            seg_input_ids = tokenizer.encode(
                seg, add_special_tokens=True if i == 0 else False
            )  # only add bos token
            seg_labels = (
                [args.ignore_index] * len(seg_input_ids)
                if current_ignore
                else seg_input_ids
            )
        ...
        labels = (
            labels + [args.ignore_index] * pad_length
        )  # padding doesn't take care of labels. do the padding here
        ...
        images.append(
            image if image.ndim == 4 else image.unsqueeze(0)
        )  # made 4 dim for image, remain 4 dim for video
    ...
    if args.clip_transform:
        ...
        inputs["pixel_values"] = torch.concat(
            images
        )  # already processed to features in dataset get item
...
def main(config):
    accelerator_log_kwargs = dict(
        log_with=config.report_to, project_dir=config.output_dir
    )
    ...
    collate_fn = functools.partial(
        preprocess, args=config.preprocess, processor=processor, collate_fn=collate_fn
    )
    train_loaders, train_media_types = setup_dataloaders(
        config, mode=config.mode, collate_fn=collate_fn
    )
    num_steps_per_epoch = math.ceil(
        sum(len(d) for d in train_loaders) / config.gradient_accumulation_steps
    )
    # load optimizer and custom scheduler
    config.scheduler.num_training_steps = num_steps_per_epoch * config.scheduler.epochs
    config.scheduler.num_warmup_steps = math.ceil(
        config.scheduler.num_training_steps * config.scheduler.warmup_ratio
    )
    ...
    if osp.exists(config.output_dir):
        ...
        if sample_saving:
            ckpt_paths = [
                subfolder
                for subfolder in subfolders
                if re.match(r"ckpt_resume_[\d.]+M$", subfolder) is not None
            ]
            ...
            ckpt_paths = [
                subfolder
                for subfolder in subfolders
                if re.match("ckpt_[^\d]+", subfolder) is not None
            ]
            ckpt_iters = [
                int(s.split(re.match("ckpt_[^\d]+", s).group())[-1]) for s in ckpt_paths
            ]
        ...
        if len(ckpt_iters) > 0:
            ...
            ckpt_path = osp.join(
                config.output_dir, ckpt_paths[ckpt_iters.index(resume_iter)]
            )
            ...
            if sample_saving:
                resume_iter = int(
                    resume_iter
                    * 1e6
                    / (config.batch_size * accelerator.state.num_processes)
                )
            ...
            accelerator.print(
                f"Resume from epoch {start_epoch}, steps{resume_cur_epoch_step}"
            )
    ...
    logger.info(
        f"Start training {str(start_time)}, from start_epoch-{start_epoch}, step-{resume_cur_epoch_step}"
    )
    ...
    if resume_cur_epoch_step > 0:
        ...
        for train_loader in train_loaders:
            skip_batch_num = int(
                (resume_cur_epoch_step / total_dta_num) * len(train_loader)
            )
            skipped_train_loader = accelerator.skip_first_batches(
                train_loader, num_batches=skip_batch_num
            )
    ...
    for epoch in range(start_epoch, config.scheduler.epochs):
        if not config.evaluate:
            ...
            for name in loss_names:
                for m in media_types:
                    metric_logger.add_meter(
                        f"{m}-{name}",
                        SmoothedValue(
                            window=config.metric_window_size, fmt="{value:.4f}"
                        ),
                    )
            ...
            for i, (media_type, inputs) in enumerate(
                iterator
            ):  # video/image, conversation, instruction, index
                ...
                if global_step % config.ckpt_steps == 0:
                    accelerator.save_state(
                        output_dir=osp.join(
                            config.output_dir, f"ckpt_resume_{resume_num_samples:.4f}M"
                        )
                    )
                    if accelerator.is_main_process:
                        for fn in os.listdir(config.output_dir):
                            if (
                                "resume" in fn
                                and fn != f"ckpt_resume_{resume_num_samples:.4f}M"
                            ):
                ...
                if global_step % config.save_steps == 0:
                    ...
                    with torch.no_grad():
                        ...
                        if not config.deepspeed:
                            save_state_dict = {
                                k: v
                                for k, v in accelerator.get_state_dict(model).items()
                        ...
                        unwrapped_model.save_pretrained(
                            osp.join(
                                config.output_dir,
                                f"pretrained_step{resume_num_samples:.4f}M",
                            ),
                ...
                if global_step % log_freq == 0:
                    ...
                    logs.update(
                        {
                            "step_loss_no_smoothing": accelerator.gather_for_metrics(
                                loss
                            )
                            .mean()
                            .item(),
                    ...
                    if accelerator.sync_gradients:
                        ...
                        accelerator.log(
                            {
                                "mini_batch_loss": accelerator.gather_for_metrics(
                                    mini_batch_loss
                                )
                                .mean()
                                .item()
        ...
        with torch.no_grad():
            ...
            if not config.deepspeed:
                ...
                save_state_dict = accelerator.get_state_dict(model)
            unwrapped_model.save_pretrained(
                osp.join(config.output_dir, f"pretrained_epoch{epoch:02d}"),
                is_main_process=accelerator.is_main_process,
                save_function=accelerator.save,
                state_dict=save_state_dict,
            )
            processor.save_pretrained(
                osp.join(config.output_dir, f"pretrained_step{epoch:02d}")
            )
            accelerator.save_state(
                output_dir=osp.join(config.output_dir, f"ckpt_epoch{epoch:02d}")
            )
