...
def single_test(model, processor, vid_path, num_frames=4, conv_mode="plain"):
    ...
    def load_video(
        video_path, num_segments=8, return_msg=True, num_frames=4, resolution=336
    ):
        ...
        num_frames = len(vr)
        print(num_frames)
        # TODO: this auto-segments frames. in our case, we have frame indices from PySceneDetect.
        ...
        if return_msg:
            # TODO: normalize fps? (safety for equal behaviour)
    ...
    query_background = (
        ...
        "# Task\n"
        "Describe the overall background in the provided video, without mentioning the actions.\n"
    )
    query_action = (
        "You are to assist me in accomplishing a task about the input video. Reply to me with a precise yet detailed response. For how you would succeed in the recaptioning task, read the following Instructions section and Then, make your response with a elaborate paragraph.\n"
        "# Instructions\n"
    ...
    # conv = conv_templates[conv_mode].copy()
    # conv.user_query(query_background, None, None, is_mm=True)
    # llm_response, conv = pllava_answer(conv=conv, model=model, processor=processor, do_sample=False, img_list=img_list, max_new_tokens=256, print_res=True)
    # conv = conv_templates[conv_mode].copy()
    # conv.user_query(query_action_base, None, None, is_mm=True)
    # XXX: these two are the same, just with different prompts (action vs. background.)
    ...
    # conv = conv_templates[conv_mode].copy()
    # conv.user_query(query_background_base, None, None, is_mm=True)
    # llm_response, conv = pllava_answer(
    #     conv=conv,
    #     model=model,
    #     processor=processor,
...
def run(rank, args, world_size):
    ...
    vid_path = "./example/1917.mp4"
    res = single_test(model, processor, vid_path, num_frames=args.num_frames)
