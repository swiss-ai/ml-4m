...
from fourm.utils.data_constants import (
    ...
    IMAGENET_SURFACE_NORMAL_STD,
    IMAGENET_SURFACE_NORMAL_MEAN,
    ...
    def get_border_functions(self, crop_points):
        ...
        def fn(x, xn):
            ...
            for diff, c in zip(c_diff, p):
                A = np.array([[diff[0], x_diff[0]], [diff[1], x_diff[1]]])
                b = x - c
                try:
...
class DetectionTransform(AbstractTransform):  # bounding boxes
    ...
    def bboxes_hflip(self, bboxes: List[Tuple], image_size: Tuple, flip: bool):
        ...
        if flip:
            bboxes = [
                tuple(A.bbox_hflip(bbox[:4], rows=image_height, cols=image_width)) + tuple(bbox[4:]) for bbox in bboxes
            ]
    ...
    def bboxes_crop_and_resize(self, bboxes: List[Tuple], crop_coords: Tuple, orig_size: Tuple):
        ...
        bboxes = [
            tuple(
                A.bbox_crop(bbox[:4], x_min=xmin, y_min=ymin, x_max=xmax, y_max=ymax, rows=orig_height, cols=orig_width)
            )
            + tuple(bbox[4:])
            for bbox in bboxes
        ]
...
class HumanPoseTransform(AbstractTransform):
    ...
    def convert_humanpose_instance(self, instances, only_pose=False):
        ...
        for ii in range(total_num_instances):
            instances_converted.append(
                ["human"]
                + (np.array(instances["bbox_xyxy"][ii]).flatten().tolist())
                + ["global"]
                + (np.array(instances["pred_smpl_params"]["global_orient"][ii]).flatten().tolist())
                + ["pose"]
                + (instances["pose_tokenized"][ii].flatten().tolist())
                + ["shape"]
                + (instances["pred_smpl_params"]["betas"][ii].flatten().tolist())
                + ["camera"]
                + (instances["pred_cam"][ii].flatten().tolist())
            )
        return instances_converted

    def humanposes_crop_and_resize(
        self,
        ...
        for instance in humanposes:
            ...
            xmin, ymin, xmax, ymax = left, top, left + w, top + h
            bbox_curr = A.bbox_crop(
                bbox_curr, x_min=xmin, y_min=ymin, x_max=xmax, y_max=ymax, rows=orig_height, cols=orig_width
            )
            bbox_curr = np.array(bbox_curr)
            ...
            if np.all(bbox_curr[1::2] > 1.0) or np.all(bbox_curr[0::2] > 1.0):  # bbox is out of range, remove it
                continue
            bbox_curr = np.clip(bbox_curr, a_min=0, a_max=1.0)
    ...
    def convert_humanposes_to_string(self, all_humanposes: List[Tuple]):
        ...
        for humanposes in all_humanposes:
            ...
            camera_params = np.clip(np.array(humanposes[36:]), a_min=-1.0, a_max=19.0)
            ...
            all_strings = [
                [pose] + pose_params_new,
                [glob] + global_orient_new,
                [camera] + camera_params_new,
                [shape] + shape_params_new,
            ]
            rand_perm = torch.randperm(4)
            instance_final = (
                [human]
                + bboxes_new
                + all_strings[rand_perm[0]]
                + all_strings[rand_perm[1]]
                + all_strings[rand_perm[2]]
                + all_strings[rand_perm[3]]
            )
    ...
    def get_query_points(self):
        if self.point_order == "grid":
            ...
            if not hasattr(self, "grid_query_points"):
                y, x = np.meshgrid(
                    np.linspace(0, self.H, self.points_per_h + 2)[1:-1],
                    np.linspace(0, self.W, self.points_per_w + 2)[1:-1],
                )
