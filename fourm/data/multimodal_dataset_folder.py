...
class MultiModalDatasetFolder(VisionDataset):
    """A generic multi-modal dataset loader where the samples are arranged in this way: ::
        ...
        root/modality_b/class_x/xxx.ext # TODO: what is meant by a class here?
    ...
    def __init__(
        ...
        samples = {
            mod: make_dataset(
                ...
                extensions,
                is_valid_file,
                cache_path=os.path.join(self.root, "dataloader_cache", f"{self.modality_paths[mod]}.pkl")
                if cache
                else None,
            )
            for mod in self.modalities
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        if self.return_path and not index in self.cache:  # noqa: E713
